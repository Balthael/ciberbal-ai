...
func BuildSyncSelection(flags SyncFlags, agentIDs []model.AgentID) model.Selection {
	...
	return model.Selection{
		...
		// Preset is set to full-pentest so selectedSkillIDs() returns the
...
func RenderSyncReport(result SyncResult) string {
	...
	if result.NoOp {
		fmt.Fprintln(&b, "ciberbal-ai sync — no managed sync actions needed")
	...
	if result.DryRun {
		fmt.Fprintln(&b, "ciberbal-ai sync — dry-run")
		fmt.Fprintf(&b, "Agents: %s\n", joinAgentIDs(result.Agents))
	...
	fmt.Fprintln(&b, "ciberbal-ai sync — managed sync executed")
