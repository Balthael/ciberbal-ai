...
// TestRunArgs_UpgradeDryRun verifies that `ciberbal-ai upgrade --dry-run` runs without
...
// TestRunArgs_UpgradeNoArgs runs `ciberbal-ai upgrade` without flags.
...
// TestRunArgs_UpgradeToolFilter verifies that `ciberbal-ai upgrade engram` filters
...
func TestRunArgs_UpgradeToolFilter(t *testing.T) {
	...
	out := buf.String()
	// Output should only mention engram or no-upgrades, not ciberbal-ai or gga.
	// This is a soft check since the tool may not be installed.
	if strings.Contains(out, "ciberbal-ai") && !strings.Contains(out, "engram") {
		t.Errorf("filtering to engram should not show ciberbal-ai in output; got: %s", out)
	}
}
...
func TestRenderUpgradeReport_PerToolSemantics_Deterministic(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantContains:   []string{"ciberbal-ai", "[--]", "source build"},
			...
			results: []upgrade.ToolUpgradeResult{
				{
					...
					NewVersion: "1.5.0",
					Status:     upgrade.UpgradeSkipped,
					ManualHint: "Download from https://github.com/Gentleman-Programming/ciberbal-ai/releases",
				},
			},
			wantContains:   []string{"ciberbal-ai", "manual update required", "github.com", "[--]"},
			wantNotContain: []string{"[!!]", "FAILED"},
		},
		{
		...
		},
		{
			...
			wantContains:   []string{"engram", "[ok]", "ciberbal-ai", "[--]", "gga", "1 succeeded", "2 skipped"},
			wantNotContain: []string{"FAILED", "[!!]"},
		},
	}
