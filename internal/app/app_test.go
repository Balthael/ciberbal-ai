...
func TestListBackupsNewestFirst(t *testing.T) {
	home := t.TempDir()
	backupRoot := filepath.Join(home, ".ciberbal-ai", "backups")
...
// TestRunArgsRestoreListIsDispatched verifies that `ciberbal-ai restore --list`
...
// TestVersionBeforeSystemGuards verifies that `ciberbal-ai version` returns the
...
func TestVersionBeforeSystemGuards(t *testing.T) {
	...
	if !strings.Contains(buf.String(), "ciberbal-ai") {
		t.Error("version output should contain 'ciberbal-ai'")
	}
}
...
// an error whose message suggests running 'ciberbal-ai help'.
func TestUnknownCommandSuggestsHelp(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "ciberbal-ai help") {
		t.Error("unknown command error should suggest 'ciberbal-ai help'")
	}
}
