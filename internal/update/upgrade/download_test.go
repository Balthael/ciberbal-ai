...
func TestFindBinaryInTar(t *testing.T) {
	...
	entries := []struct {
		...
		{"ciberbal-ai_1.5.0_darwin_arm64/ciberbal-ai", content}, // binary in subdir
	}
	...
	outPath := filepath.Join(t.TempDir(), "ciberbal-ai")
	...
	err := downloadBinary(context.Background(), server.URL+"/release.tar.gz", "ciberbal-ai", outPath)
	if err != nil {
		t.Fatalf("downloadBinary: %v", err)
	}
