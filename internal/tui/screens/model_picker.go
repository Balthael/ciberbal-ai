...
// SDDOrchestratorPhase is the key used for the ciberbal (orchestrator) model assignment.
const SDDOrchestratorPhase = "ciberbal"

// ModelPickerRows returns the row labels for the model picker screen.
// Row 0 is "ciberbal" (coordinator), row 1 is "Set all phases",
...
func handleModelNav(
	...
	case "enter":
		...
		case state.SelectedPhaseIdx == 0:
			// "ciberbal" row — assign only to the orchestrator key
...
func renderPhaseList(
	...
	if len(state.AvailableIDs) == 0 {
		...
		b.WriteString("\n")
		b.WriteString(styles.SubtextStyle.Render("Run 'opencode' once, then re-run 'ciberbal-ai sync' to assign models."))
		b.WriteString("\n")
	...
	for idx, row := range rows {
		...
		switch {
		case idx == 0:
			// "ciberbal" row — coordinator, individual assignment only
