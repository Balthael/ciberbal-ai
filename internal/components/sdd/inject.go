...
type InjectOptions struct {
	...
	// <!-- ciberbal-ai:strict-tdd-mode --> marker section is injected into
...
//
// Walking upward means users can run ciberbal-ai from any subdirectory of their
...
func Inject(homeDir string, adapter agents.Adapter, sddMode model.SDDModeID, options ...InjectOptions) (InjectionResult, error) {
	...
	// findProjectRoot walks upward from WorkspaceDir so ciberbal-ai can be
...
func hasLegacyBareOrchestrator(content string) bool {
	markedIdx := strings.Index(content, "<!-- ciberbal-ai:sdd-orchestrator -->")
	if markedIdx >= 0 {
...
//   - end at the next managed marker ("<!-- ciberbal-ai:") if present, else EOF
...
func stripBareOrchestratorForFilePrompt(content string) string {
	if markedIdx := strings.Index(content, "<!-- ciberbal-ai:sdd-orchestrator -->"); markedIdx >= 0 {
		prefix := content[:markedIdx]
	...
	if rel := strings.Index(content[start:], "<!-- ciberbal-ai:"); rel >= 0 {
		end = start + rel
	}
...
func injectMarkdownSections(homeDir string, adapter agents.Adapter, assignments map[string]model.ClaudeModelAlias) (InjectionResult, error) {
	...
	if hasSDDOrchestrator(existing) && !strings.Contains(existing, "<!-- ciberbal-ai:sdd-orchestrator -->") {
...
func injectClaudeModelAssignments(content string, assignments map[string]model.ClaudeModelAlias) (string, error) {
	const openMarker = "<!-- ciberbal-ai:sdd-model-assignments -->"
	const closeMarker = "<!-- /ciberbal-ai:sdd-model-assignments -->"
