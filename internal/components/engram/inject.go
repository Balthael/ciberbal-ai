...
func Inject(homeDir string, adapter agents.Adapter) (InjectionResult, error) {
	...
	case model.StrategySeparateMCPFiles:
		...
		// `engram setup <agent>` is invoked. ciberbal-ai's Inject() runs after
		...
		// See: https://github.com/Gentleman-Programming/ciberbal-ai/issues (engram absolute path regression)
...
// ciberbal-ai runs Inject() after setup, so we must not overwrite that absolute
