...
<#
.SYNOPSIS
    ciberbal-ai — Install Script for Windows
    One command to configure your cybersecurity AI stack on any OS.

.DESCRIPTION
    Downloads and installs the ciberbal-ai binary for Windows.
...
.EXAMPLE
    ...
    # Or download and run:
    Invoke-WebRequest -Uri https://raw.githubusercontent.com/Balthael/ciberbal-ai/main/scripts/install.ps1 -OutFile install.ps1
    .\install.ps1
...
function Show-Banner {
    ...
    Write-Host ""
    Write-Host "  One command to configure your cybersecurity AI stack on any OS" -ForegroundColor DarkGray
    Write-Host ""
}
...
function Install-ViaBinary {
    ...
    try {
        ...
        if (-not $installDir) {
            $installDir = Join-Path $env:LOCALAPPDATA "ciberbal-ai\bin"
        }
...
function Test-Installation {
    ...
    $locations = @(
        (Join-Path $env:LOCALAPPDATA "ciberbal-ai\bin\$BINARY_NAME.exe")
    )
    if ($gopath) {
